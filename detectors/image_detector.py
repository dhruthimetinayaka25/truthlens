...
class ImageForensicDetector:
    ...
    def compute_sha256(raw_bytes: bytes) -> str:
        """Cryptographic SHA-256 checksum for forensic chain of custody."""
    ...
    def compute_fft_spectrum(self, original_img: Image.Image) -> tuple[Image.Image, float]:
        ...
        Exposes high-frequency periodic grid artifacts generated by GAN/Diffusion deconvolution layers.
        """
        ...
        # Radial high-frequency energy ratio metric
        h, w = gray.size
        cy, cx = h // 2, w // 2
        r = min(h, w) // 4
        y, x = np.ogrid[:w, :h]
        mask = ((x - cx)**2 + (y - cy)**2) > r**2
        hf_energy_ratio = float(np.mean(np.abs(f_shift)[mask.T]))
